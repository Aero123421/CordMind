...
- Bot が “観測→計画→実行→再計画” の多段階タスクに対応し、ID をユーザーに要求しない方向へ寄せる
- 確認 UI は JSON 丸出しにせず、人間が読める要約で提示する
...
- M5: 監査ログ / レート制限 / 運用項目
- M6: MVP 受入テスト
- M7: エージェントループ（観測→再計画）と評価（Evals）で品質を回す

## 追調査が必要な項目
- 5 プロバイダーのモデル ID / 料金 / レート制限の最終確定
- 破壊的判定の最終ルール（AI 判断 + ルール併用の細部）
- プロンプト注入耐性と “untrusted input” の取り扱い（ガードレール設計）
- 代表ログを使った評価（Evals）の設計と、回帰防止の運用手順
